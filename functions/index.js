...
class FirestoreSessionStore {
    ...
    async get(sid, callback) {
        try {
            const session = await this.sessions.doc(sid).get();
            callback(null, session.exists ? session.data() : null);
        } catch (err) {
            callback(err);
        }
    }
    ...
    async set(sid, session, callback) {
        try {
            await this.sessions.doc(sid).set(session);
            callback();
        } catch (err) {
            callback(err);
        }
...
// Удаляем app.listen и экспортируем функцию
